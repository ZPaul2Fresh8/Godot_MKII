...
# TODO: CREATE .END FILES FOR ANIM COMMANDS. FORMAT SHOULD BE "0-9.END"
# EXAMPLE ANI_COMMAND 1 = JUMP TO ANOTHER POSTION IN ANIMATION SO WE'D EXPECT
# TO SEE A FILE NAME "1.2.END" IF WE WERE JUMPING TO FRAME 2 AT THE END OF
# THIS ANIMATION. THESE ARE ALL STRING PARSED ATM.
...
# AND OTHERS THAT NEED FATALITY PALETTE APPLIED INSTEAD OF NORMAL PALETTE
	...
	for char_id in 1:							# Extract just Lao for now
		...
		# FORCE SPECIFIC PALETTE - NEEDED FOR CLONED FIGHTERS
		# 0x20F22 - PRIMARY PALETTE ARRAY
		# 0x20F66 - ALTERNATE PALETTE ARRAY
		# 0x21920 - FATALITY PALETTE ARRAY\
		#var palette : PackedColorArray = Tools.Convert_Palette(Tools.Get_Pointer(char_id * 4 + 0x20f22))
		#var fpalette : PackedColorArray = Tools.Convert_Palette(Tools.Get_Pointer(char_id * 4 + 0x21920))
		...
		for anim_id in ani_count:
			...
			while Tools.Get_Long(frame) != 0:
				...
				if char_id == 8:
					Tools.palette = Tools.Convert_Palette(Tools.Get_Pointer(char_id * 4 + 0x20f22))
				
				# CHECK IF MULTI-SEGMENTED FRAME OR NOT BY LOOKING AT PTR VALUE
				var multi : bool = true
				if !Tools.Is_Frame_MultiSegmented(frame):
					# SINGLE SEGMENT FRAME DETECTED SINCE
					print("Single segment frame detected @" + str(frame_ptr))
					multi = false
